...
import adminEofMusicTracks from '../backend/api/admin/eof-music-tracks.js'
import adminEofScheduler from '../backend/api/admin/eof-scheduler.js'
...
import authResendEntryEmail from '../backend/api/auth/resend-entry-email.js'
import authDeleteAccount from '../backend/api/auth/delete-account.js'

/**
 * Lazy-load heavy EOF Production / Script Maker handlers so login/auth cold starts
 * do not evaluate the ffmpeg video graph (and NFT-linked native binaries).
 * Import / handler failures return JSON instead of bare FUNCTION_INVOCATION_FAILED.
 * @param {() => Promise<{ default: Function }>} importer
 * @param {string} [label]
 */
function lazyDefault(importer, label = 'route') {
  return async (req, res) => {
    try {
      ...
      if (typeof handler !== 'function') {
        ...
        res.end(JSON.stringify({ error: `EOF handler missing export for ${label}` }))
        return
      }
      return await handler(req, res)
    } catch (e) {
      console.error(`[vercel-dispatch] lazy ${label}`, e)
      if (res.headersSent || res.writableEnded) return
      res.statusCode = 500
      res.setHeader('Content-Type', 'application/json')
      res.end(
        JSON.stringify({
          error: e instanceof Error ? e.message : `Failed to load ${label}`,
          code: 'EOF_HANDLER_LOAD_FAILED',
        }),
      )
    }
  }
...
export const routes = {
  ...
  // Heavy EOF video / script pipelines — deferred load (see lazyDefault).
  '/api/admin/eof-production': lazyDefault(
    () => import('../backend/api/admin/eof-production.js'),
    'eof-production',
  ),
  '/api/admin/eof-production-audio': lazyDefault(
    () => import('../backend/api/admin/eof-production-audio.js'),
    'eof-production-audio',
  ),
  '/api/admin/eof-production-video': lazyDefault(
    () => import('../backend/api/admin/eof-production-video.js'),
    'eof-production-video',
  ),
  '/api/admin/eof-production-scene-image': lazyDefault(
    () => import('../backend/api/admin/eof-production-scene-image.js'),
    'eof-production-scene-image',
  ),
  '/api/admin/eof-music-tracks': adminEofMusicTracks,
  '/api/admin/eof-scheduler': adminEofScheduler,
  '/api/admin/eof-script-maker': lazyDefault(
    () => import('../backend/api/admin/eof-script-maker.js'),
    'eof-script-maker',
  ),
  '/api/eof-daily-cron': lazyDefault(() => import('../backend/api/eof-daily-cron.js'), 'eof-daily-cron'),
  '/api/eof-script-maker-cron': lazyDefault(
    () => import('../backend/api/eof-script-maker-cron.js'),
    'eof-script-maker-cron',
  ),
  ...
  '/api/auth/resend-entry-email': authResendEntryEmail,
  '/api/auth/delete-account': authDeleteAccount,
}
...
export async function dispatch(req, res, path) {
  ...
  try {
    ...
    console.error('[vercel-dispatch]', path, e)
    if (res.headersSent || res.writableEnded) return
    res.statusCode = 500
    res.setHeader('Content-Type', 'application/json')
    res.end(
