...
describe('vercelApiDispatch routing', () => {
  ...
  it('routes auth register under /api/auth prefix', () => {
    const req = { url: '/api/auth/register', query: { slug: ['register'] } }
    assert.equal(pathFromRequest(req, '/api/auth'), '/api/auth/register')
  })
})
