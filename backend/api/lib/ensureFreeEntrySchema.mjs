...
let ensured = false
let entryLogsEnsured = false

/** Entry-attempt log only — safe on fresh Neon (no users/tickets FK). */
export async function ensureEntryAttemptLogSchema() {
  if (entryLogsEnsured) return
...
export async function ensureFreeEntrySchema() {
  ...
  if (dbIsPostgres()) {
    try {
      ...
      console.warn('[schema] free_online_entries skipped (run backend/api/db/schema.sql on Postgres):', e?.message || e)
    }
