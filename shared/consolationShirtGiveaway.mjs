/** Automatic shirt giveaway entries when Signed Legacy Bundle skill answers are incorrect. */
...
export const CONSOLATION_PRIZE_SUMMARY =
  'If you get the skill questions wrong, you automatically receive 2 entries into the separate Free Ronaldo Shirt Giveaway (consolation prize). This does not qualify you for the main Signed Legacy Bundle draw.'
...
export const CONSOLATION_PRIZE_FREE_APPLIES =
  'Free online Signed Legacy Bundle entrants who get the skill questions wrong receive the same 2 consolation shirt entries (no spend threshold).'
...
export function formatConsolationAwardMessage({ entryCount = CONSOLATION_SHIRT_ENTRY_COUNT } = {}) {
  ...
  return `As a consolation prize, you have received ${entryCount} automatic ${noun} into the separate Free Ronaldo Shirt Giveaway. This does not enter you into the main Signed Legacy Bundle draw. ${CONSOLATION_NO_REFUND_REMINDER}`
}

export const CONSOLATION_NOT_AWARDED_PAID_BELOW_THRESHOLD =
  'You do not qualify for the main Signed Legacy Bundle draw on this attempt. Consolation shirt entries apply only when you spend £10 or more on tickets in a single purchase. Tickets are not refunded.'

export const CONSOLATION_NOT_AWARDED_GENERIC =
  'You do not qualify for the main Signed Legacy Bundle draw on this attempt. Tickets are not refunded.'
