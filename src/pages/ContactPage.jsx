...
export default function ContactPage() {
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      if (!res.ok) {
        const fallback =
          res.status === 503 && data.contactEmail
            ? `Contact form is temporarily unavailable. Email us at ${data.contactEmail}.`
            : data.error || `Could not send (HTTP ${res.status}).`
        setError(fallback)
        setStatus('error')
        return
      }
      ...
      <div className="mx-auto max-w-3xl px-4 pb-16 pt-14 sm:px-6 sm:pb-20 sm:pt-20">
        ...
        {status === 'sent' ? (
          ...
          >
            ...
            <div className="px-6 py-8 sm:px-8 sm:py-10">
              ...
              <p className="mt-3 max-w-md text-sm leading-relaxed text-emerald-200/85 sm:text-base">
                Thank you. Your message has been submitted and we will respond to your email address as soon as we
                can.
              </p>
              <button
                type="button"
              ...
              <Link
                to="/competitions"
                className="ml-3 mt-6 inline-flex items-center rounded-xl border border-white/10 px-4 py-2.5 text-sm font-semibold text-stone-300 transition hover:bg-white/5"
              >
          ...
          <div className="mt-10 overflow-hidden rounded-2xl border border-teal-500/25 bg-stone-950/60 shadow-[0_20px_50px_rgba(0,0,0,0.35)]">
            ...
            <form onSubmit={onSubmit} noValidate className="ss-contact-form space-y-6 px-5 py-8 sm:px-8 sm:py-10">
              <input
                type="text"
                name="company"
                value={company}
              ...
              <div className="border-t border-white/[0.06] pt-6">
                ...
                <p className="mt-4 text-center text-xs leading-relaxed text-stone-600">
                  We typically reply by email. For urgent payment issues, include your order reference if you have
                  one.
                </p>
              </div>
            </form>
